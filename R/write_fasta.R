...
#' @param contain_flag if the name on the first elements of the lists already contain
...
write_fasta <- function(fasta_list = fasta_list, named = FALSE, contain_flag = TRUE,
  ...
  if (combined == TRUE & length(path_to_file) > 1) {
    stop("Error write_fasta: the `path_to_file` has length greater than 1 and the
